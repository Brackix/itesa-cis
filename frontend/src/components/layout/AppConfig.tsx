    ...
    return (
        <>
            ...
            <Sidebar visible={layoutState.configSidebarVisible} onHide={onConfigSidebarHide} position="right" className="layout-config-sidebar w-20rem">
                ...
                <div className="grid">
                    {[
                        { theme: 'lara-light-indigo', scheme: 'light', label: 'Lara Light Indigo' },
                        { theme: 'lara-light-blue', scheme: 'light', label: 'Lara Light Blue' },
                        { theme: 'lara-light-purple', scheme: 'light', label: 'Lara Light Purple' },
                        { theme: 'lara-light-teal', scheme: 'light', label: 'Lara Light Teal' },
                        { theme: 'lara-dark-indigo', scheme: 'dark', label: 'Lara Dark Indigo' },
                        { theme: 'lara-dark-blue', scheme: 'dark', label: 'Lara Dark Blue' },
                        { theme: 'lara-dark-purple', scheme: 'dark', label: 'Lara Dark Purple' },
                        { theme: 'lara-dark-teal', scheme: 'dark', label: 'Lara Dark Teal' },
                    ].map(({ theme, scheme, label }) => (
                        <div key={theme} className="col-3">
                            <button className="p-link w-2rem h-2rem" onClick={() => _changeTheme(theme, scheme)}>
                                <img src={`/layout/images/themes/${theme}.png`} className="w-2rem h-2rem" alt={label} />
                            </button>
                        </div>
                    ))}
                </div>
